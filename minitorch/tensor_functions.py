...
#         grad_input = softmax_out.f.attn_softmax_bw(out_grad, softmax_out)
...
class Attn_Softmax(Function):
    ...
    def forward(ctx: Context, inp: Tensor, mask: Tensor) -> Tensor:
        ...
        # 儲存 softmax 輸出和 mask（mask 用於確定其形狀，以返回零梯度）
        ctx.save_for_backward(softmax_out, mask)
        return softmax_out

    @staticmethod
    def backward(ctx: Context, out_grad: Tensor) -> Tuple[Tensor, Tensor]:
        # 從上下文中取出保存的 softmax 輸出與 mask
        (softmax_out, mask) = ctx.saved_tensors
        # 計算輸入的梯度
        grad_input = softmax_out.f.attn_softmax_bw(out_grad, softmax_out)
        # 對於 mask，通常不需要梯度，返回一個全零 tensor
        grad_mask = zeros(mask.shape, backend=mask.backend)
...
class LayerNorm(Function):
    ...
    def forward(ctx: Context, inp: Tensor, gamma: Tensor, beta: Tensor) -> Tensor:
        #   BEGIN ASSIGN3_2
        normalized_output = inp.f.layernorm_fw(inp, gamma, beta)
        # ctx.save_for_backward(inp, gamma, beta)
        ctx.save_for_backward(inp, gamma, beta, normalized_output.var, normalized_output.mean)
        return normalized_output
        #   END ASSIGN3_2

    @staticmethod
    def backward(ctx: Context, out_grad: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        inp, gamma, beta, var, mean = ctx.saved_tensors
        grad_input = inp.f.layernorm_bw(out_grad, inp, gamma, beta, var, mean)
        
        # 將 mean 與 var 從 (batch_size,) 展開為 (batch_size, 1)
        mean_expanded = mean.view(mean.shape[0], 1)
        var_expanded = var.view(var.shape[0], 1)
        
        # 計算正規化輸入：normalized = (inp - mean) / sqrt(var)
        # 替換 rsqrt(var) 為 (var ** (-0.5))
        normalized = (inp - mean_expanded) * (var_expanded ** (-0.5))
        
        # 計算 gamma 與 beta 的梯度
        grad_gamma = (out_grad * normalized).sum(dim=0)
        grad_beta = out_grad.sum(dim=0)
