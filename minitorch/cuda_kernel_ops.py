...
class CudaKernelOps(TensorOps):
    ...
    def attn_softmax_bw(out_grad: Tensor, soft_inp: Tensor):
        ...
        batch_size = soft_inp.shape[0]
        nhead = soft_inp.shape[1]
        from_len = soft_inp.shape[2]
        to_len = soft_inp.shape[3]
        # the number of each softmax vec
        int_rows = batch_size * nhead * from_len
        ...
        lib_softmax.launch_attn_softmax_bw(
            ctypes.c_void_p(out_grad._tensor._storage.ctypes.data),
            ctypes.c_void_p(soft_inp._tensor._storage.ctypes.data),
            int_rows,
            to_len,
            stream
        )
    ...
    def layernorm_fw(inp: Tensor, gamma: Tensor, beta: Tensor):
        ...
        vars_  = inp.zeros((batch_size,))
        means_ = inp.zeros((batch_size,))
